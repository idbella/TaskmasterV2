...
/*   Updated: 2019/09/24 19:59:04 by sid-bell         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "parser.h"

void	ft_getentrys(t_app *app, t_entry *entry, char *strval)
{
...
void	ft_getstring(t_app *app, t_entry *entry, t_jsobject *js)
{
	...
	if (ft_isvalid(entry, js, json_type_string))
	{
		...
		if (ft_strequ(entry->name, "cmd"))
		{
			...
			{
				ft_printf_fd(1,
					"[!] Error : command %s must have an absolute path\n",
					app->argv[0]);
				exit(0);
			}
		}
		ft_getentrys(app, entry, strval);
	}
}
